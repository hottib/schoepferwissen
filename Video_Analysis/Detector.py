...
class Detector:
    ...
    def onVideo(self,showVideo= True,  writeVideo = False):
        ...
        while success:
            ...
            if len(bboxIdx) != 0:
                for i in range(0, len(bboxIdx)):
                    ...
                    classLabel = self.classesList[classLabelID].upper()
                    classColor = [int(c) for c in self.colorList[classLabelID]]
                    if classLabel not in labelCollection:
                        labelCollection.append(classLabel)
            ...
            cv2.putText(image, "RENDERING with FPS: " + str(int(fps)), (20,70), cv2.FONT_HERSHEY_PLAIN, 2, (0,255,0), 2)
            if writeVideo == True: 
                result.write(image)
            if showVideo == True:
